...
export class TimeCalculator {
  private readonly rules: FlexTimeRules = {
   ...
   * 計算預計下班時間
   */
  calculateExpectedClockOut(clockInTime: string): ExpectedClockOut {
    ...
    try {
      ...
      let rule: ExpectedClockOut['rule']
      ...
      return {
        ...
        workingMinutes: 0,
        rule: 'error',
        description: `計算錯誤: ${error instanceof Error ? error.message : '未知錯誤'}`
      }
    }
  }
   ...
   * 計算剩餘工作時間
   */
  calculateRemainingTime(clockInTime: string, currentTime: Date | null = null): RemainingTime {
    if (!clockInTime || clockInTime === '--:--') {
      return {
        remainingTime: '--:--',
        remainingMinutes: 0,
    ...
    try {
      ...
      const remainingMinutes = (expectedClockOut.expectedClockOutMinutes || 0) - currentMinutes
   ...
   * 分析上班時間類型
   */
  analyzeClockInTime(clockInTime: string): ClockInAnalysis {
    if (!clockInTime || clockInTime === '--:--') {
      return {
        type: 'none',
        description: '尚未打卡',
   ...
   * 取得彈性上班制度說明
   */
  getFlexTimeRules(): FlexTimeRulesDescription {
    return {
      title: '彈性上班制度規則',
      rules: [
        {
   ...
   * 將時間物件轉換為分鐘數
   */
  private timeToMinutes(timeObj: TimeObject): number {
   ...
   * 檢查是否為工作日
   */
  isWorkingDay(date: Date | null = null): boolean {
